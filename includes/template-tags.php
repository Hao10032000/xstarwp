...
function themesflat_posted_category( $layout = '' ) { 	
	if ( has_category() ) {
		echo '<div class="post-categories">'.esc_html__("In - ",'xstar');
			the_category( ', ' );
		echo '</div>';
	}	
}
...
function themesflat_entry_footer() {
	...
	if ( 'post' == get_post_type() ) {
		...
		if ( $tags_list && is_single() ) {
			$tags_links = sprintf( '<div class="tags-links"><h5>' . esc_html__( 'Tags', 'xstar' ) . '</h5>' . esc_html__( ' %1$s', 'xstar' ) . '</div>', $tags_list  );
		...
		<nav class="navigation posts-navigation" role="navigation">
			...
			<ul class="nav-links clearfix">
				...
				if ( is_attachment() ) :
					...
					$prev = esc_html__( 'Published In', 'xstar' );
					$date = get_the_date();
					echo '<li class="post-navigation previous-post">';
